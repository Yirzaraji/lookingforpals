...
<?php if(!isset($_GET['article']) || $state == "ok"):?>
    <div class="post-save-options messages-container"> 
        ...
        <input type="submit" id="edit-article" class="button small" value="<?php _e("Edit article", "social-articles"); ?>" />
        <input type="submit" id="view-article" class="button small" value="<?php _e("View article", "social-articles"); ?>" />
        <input type="submit" id="new-article" class="button small" value="<?php _e("New article", "social-articles"); ?>" />
    </div>
    <div id="post-maker-container">
        ...
        <div class="buttons-container" id="create-controls">
            <?php if(($status=="draft" || $status == "new-post") && !$directWorkflow):?>
                <label for="publish-save"><input type="checkbox" id="publish-save" /> <span></span><?php _e("Save and move it under review", "social-articles"); ?></label>
            <?php endif?>
            <?php if(($status=="draft" || $status == "new-post") && $directWorkflow):?>
                <input type="checkbox" id="publish-save" /><label for="publish-save"><span></span><?php _e("Save and publish", "social-articles"); ?></label>
            <?php endif?>

            <input type="submit" class="button radius save" value="<?php _e("Save", "social-articles"); ?>" onclick="savePost(); return false;" />
            <input type="submit" class="button radius cancel" value="<?php _e("Cancel", "social-articles"); ?>" onclick="window.open('<?php echo $bp->loggedin_user->domain.'/articles';?>', '_self')" />
        </div>  
    </div>    
<?php else:?>
